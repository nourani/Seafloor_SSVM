...
int main(int argc, char* argv[]) {
	...
	read_input_parameters(argc, argv, trainfile, modelfile, &verbosity,
			&struct_verbosity, &struct_parm, &learn_parm, &kernel_parm,
			&alg_type);
	svm_struct_init_hierarchy(&struct_parm);
	...
	weighted_loss = 0;
	if( weighted_loss ) {
		printf("\tUsing weighted loss\n");
	}
	/* Do the learning and return structmodel. */
	if (alg_type == 0)
		svm_learn_struct(sample, &struct_parm, &learn_parm, &kernel_parm,
				&structmodel, NSLACK_ALG);
	else if (alg_type == 1)
		svm_learn_struct(sample, &struct_parm, &learn_parm, &kernel_parm,
				&structmodel, NSLACK_SHRINK_ALG);
	else if (alg_type == 2)
		...
		svm_learn_struct_joint_custom(sample, &struct_parm, &learn_parm,
...
void read_input_parameters(int argc, char *argv[], char *trainfile,
		char *modelfile, long *verbosity, long *struct_verbosity,
...
void print_help() {
	...
	printf(
			"         -w [0,..,9] -> choice of structural learning algorithm (default %d):\n",
